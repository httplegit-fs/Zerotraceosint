...
function proxyPlugin(): Plugin {
  return {
    name: 'vite-plugin-cyber-proxy',
    configureServer(server) {
      server.middlewares.use(async (req, res, next) => {
        if (!req.url) return next();

        // 1. Phone Lookup Proxy
        if (req.url.startsWith('/api/phone-lookup') || req.url.startsWith('/api/proxy') || req.url.startsWith('/api/lookup')) {
          let phone = '';
          try {
            ...
            // Fallback to alonepatel endpoint
            try {
        ...
        if (req.url.startsWith('/api/email-lookup') || req.url.startsWith('/api/email')) {
          ...
          try {
            ...
            let targetUrl = '';
            if (customTarget && customTarget.includes('trycloudflare.com')) {
              // Exact format: https://storage-deutschland-don-patterns.trycloudflare.com/email?value=rajkumar58@gmail.com&key=DADDY
            ...
            // Fallback to alonepatel email endpoint if cloudflare connection fails
            try {
        ...
        if (req.url.startsWith('/api/auth/verify')) {
          ...
          req.on('end', () => {
            try {
              ...
              const granted = pin === '2007' || pin === '9161';
              res.setHeader('Content-Type', 'application/json');
              res.end(JSON.stringify({ granted, currentPin: '2007', message: granted ? 'ACCESS GRANTED' : 'ACCESS DENIED' }));
            } catch {
              res.setHeader('Content-Type', 'application/json');
              res.end(JSON.stringify({ granted: false, message: 'Invalid format' }));
            }
          });
          return;
        }
