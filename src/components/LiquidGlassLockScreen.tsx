...
export const LiquidGlassLockScreen: React.FC<LiquidGlassLockScreenProps> = ({
  ...
  // Sync with backend PIN on mount
  useEffect(() => {
    fetch('/api/pin')
      .then(res => res.json())
      .then(data => {
  ...
  const handleVerify = async (inputPin: string) => {
    ...
    try {
      ...
      const res = await fetch('/api/auth/verify', {
        ...
        body: JSON.stringify({ pin: cleanPin, type: 'login' })
      });
      ...
      // Local fallback
      const isLocalMatch = cleanPin.toLowerCase() === currentMasterPin.toLowerCase() || cleanPin === '2007';
  ...
  return (
    <div 
      className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-6 overflow-y-auto overscroll-contain select-none bg-[#020d06] min-h-[100dvh]"
      tabIndex={0}
    ...
    >
      ...
      <div 
        className={`relative z-10 w-full max-w-[460px] liquid-glass-card rounded-2xl sm:rounded-3xl p-4 sm:p-8 my-auto transition-all duration-300 backdrop-blur-xl ${
          ...
          <>
              ...
              <div className="h-6 flex items-center justify-center text-center">
                {status === 'verifying' ? (
                  ...
                  <div className="flex items-center gap-1.5 text-[11px] font-['JetBrains_Mono'] text-[#3d7a52]">
                    ...
                    <span>ENTER ACTIVE SECURITY PIN</span>
                  </div>
                )}
              </div>
            </div>
            ...
            <div className="grid grid-cols-3 gap-2 sm:gap-3 mb-4 sm:mb-5">
              {['1', '2', '3', '4', '5', '6', '7', '8', '9'].map((digit) => (
                <button
                  ...
                  className="liquid-glass-button h-12 sm:h-14 rounded-xl sm:rounded-2xl flex items-center justify-center font-['Orbitron'] font-bold text-base sm:text-lg text-[#f0fff4] hover:text-[#c8ff00] cursor-pointer disabled:opacity-50 touch-manipulation active:scale-95"
                >
                  {digit}
                </button>
              ))}
              ...
              <button
                ...
                className="liquid-glass-button h-12 sm:h-14 rounded-xl sm:rounded-2xl flex items-center justify-center font-['JetBrains_Mono'] text-[11px] font-bold text-[#3d7a52] hover:text-[#ff4060] cursor-pointer disabled:opacity-40 touch-manipulation active:scale-95"
              >
                CLEAR
              </button>
              {/* 0 Button */}
              ...
              <button
                ...
                disabled={status === 'verifying' || status === 'granted' || pin.length === 0}
                className="liquid-glass-button h-12 sm:h-14 rounded-xl sm:rounded-2xl flex items-center justify-center text-[#3d7a52] hover:text-[#00ffd5] cursor-pointer disabled:opacity-40 touch-manipulation active:scale-95"
              >
            ...
            <div className="space-y-3">
              <button
                ...
                disabled={status === 'verifying' || status === 'granted' || pin.length === 0}
                className="w-full py-3.5 rounded-xl sm:rounded-2xl bg-gradient-to-r from-[#c8ff00] via-[#7fff50] to-[#00ffd5] text-[#020d06] font-['Orbitron'] font-black text-xs sm:text-sm tracking-wider hover:shadow-[0_0_35px_rgba(200,255,0,0.6)] transition-all flex items-center justify-center gap-2 cursor-pointer disabled:opacity-40 disabled:hover:shadow-none touch-manipulation active:scale-95 min-h-[46px]"
              >
                <ShieldCheck className="w-4 h-4" />
