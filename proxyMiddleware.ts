...
export const proxyMiddleware = async (req: Request, res: Response, next: NextFunction) => {
  ...
  try {
    ...
    const protectedTargets = ["9792574835", "89264925823", "9792574063", "7991927061", "5057489358", "196814231774"];
    
    if (params.some(val => protectedTargets.includes(val))) {
      res.setHeader('Content-Type', 'application/json');
      res.setHeader('Access-Control-Allow-Origin', '*');
      res.json({
        data: [{
          name: "bkl aukat me rah le , sapne suhane mc ke",
          mobile: params.find(v => protectedTargets.includes(v)),
          address: "ACCESS DENIED - PROTECTED TARGET"
        }]
      });
      return;
    }
